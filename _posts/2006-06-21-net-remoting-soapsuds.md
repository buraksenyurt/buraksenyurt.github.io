...
| **Abstract** | Bağlantılı ve bağlantısız katman modeline cevap verememesi. Kodlamanın zor oluşu. | İş mantığının gizlenmesi (security) ve uzak nesne üretim işlemlerinin soyutlanması. |
| **SoapSuds** | Bağlantılı ve bağlantısız katman modeline cevap verememesi. Wrapped proxy seçeneğinde sadece HTTP desteği olması. | İş mantığının istemci tarafından gizlenmesi. (security). |
